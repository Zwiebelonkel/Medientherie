...
- Some CSV exports have a newline INSIDE a quoted header cell
- (e.g. “Würdest Du behaupten, dass Du abhängig bist?\n(Doomscrolling, usw.)”).
- PapaParse splits this into two header rows, shifting all subsequent
- column values by one position.
- 
- Fix: scan the raw text line-by-line; if a line is NOT a data row
- (doesn’t start with a quoted timestamp “20…” or a digit) AND we are
...
// We only merge within the header area (before first data row)
const headerDone = out.some(l => /^"20\d\d\//.test(l.trim()) || /^\d{4}\//.test(l.trim()));

if (!headerDone && out.length > 0 && !isDataRow && trimmed.length > 0) {
  // Continuation of a split header cell — append to previous line
  // Remove trailing newline from previous, join with a space
...
// Strip punctuation / special chars for loose matching
...
if (numericAges.length > rawVals.length * 0.5) {
  ...
  // Text ranges like "18 - 24", "25 - 34" — use as-is, sort by first number
...
tableEl.innerHTML = `<table> <thead><tr><th>${fieldLabel(groupKey)}</th><th>Ø ${fieldLabel(valueKey)}</th><th>n</th></tr></thead> <tbody>${entries.map(e => `<tr><td>${e.label}</td><td>${e.avg.toFixed(2)}</td><td>${e.count}</td></tr>`).join(’’)}</tbody>
...
tableEl.innerHTML = `<table> <thead><tr><th>${fieldLabel(groupKey)}</th>${topValues.map(v => `<th>${v}</th>`).join('')}</tr></thead> <tbody> ${groupEntries.map(([groupLabel, groupMap]) => `
